...
| OriginalLogSeverity | string | A non-mapped version of LogSeverity. For example: Warning/Critical/Info instead of the normalized Low/Medium/High in the LogSeverity Field |
...
| ReceiptTime | string | The time at which the event related to the activity was received. Different then the 'Timegenerated' field, which is when the event was received in the log collector machine. |
